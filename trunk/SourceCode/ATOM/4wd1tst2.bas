...
left_speed = 300		' Left Scorpion stop value.
right_speed = 300		' Right Scorpion stop value.

low	p12				' Ensure pulsout commands are positive going.
low	p13
...
main:
if in4 = 0 then right_adjust	'A button increments then decrements right_speed variable by 10% each press.
if in5 = 0 then reset_both		'B button resets both speed variables to stopped.  
if in6 = 0 then left_adjust		'C button increments then decrements left_speed variable by 10% each press.
' Send out the servo pulses	
	pulsout 12,(left_speed*10)	' Left Scorpion channel.
	pulsout 13,(right_speed*10)	' Right Scorpion channel.
	pause 20
